...
ylim([-10 90]);
shading interp;
load('CustomColormap for Fig1c.mat');
...
x = 0;  
y = -34;  
text(x, y, 'SWE bias (kg m^-^2)', 'FontSize', 14, 'HorizontalAlignment', 'center');
caxis([-50.2 50.2]);
newFontSize = 14;
set(gca, 'FontSize', newFontSize);
aa = load('significance_mask for Fig1c.mat');
bb = aa.Test_result;
xline = -178:0.2:178;
hold on;
nsize = length(xline);
