...
ylim([-10 90]);
shading interp;
load('CustomColormap for Fig1e.mat');
colormap(CustomColormap);
h = colorbar('Location', 'southoutside','YLim',[-30 30],'YTick',[-30 -20 -10 0 10 20 30]);
h.Position = [0.1 0.1 0.8 0.05]; %[left bottom width height]
set(h, 'FontSize', 14);
x = 0;  
y = -34;  
text(x, y, 'Light snowfall frequency bias (days)', 'FontSize', 14, 'HorizontalAlignment', 'center');
caxis([-30.2 30.2]);
newFontSize = 14;
set(gca, 'FontSize', newFontSize);
aa = load('significance_mask for Fig1e.mat');
bb = aa.Test_result;
xline = -178:0.2:178;
hold on;
nsize = length(xline);
