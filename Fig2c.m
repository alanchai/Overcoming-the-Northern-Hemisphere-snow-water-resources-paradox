...
ylim([-10 90]);
shading interp;
load('CustomColormap for Fig2c.mat');
colormap(CustomColormap);
h = colorbar('Location', 'southoutside','YLim',[10 50],'YTick',[10 20 30 40 50]);
h.Position = [0.1 0.075 0.8 0.05]; %[left bottom width height]
caxis([9.75 50.25]);
x = 0;  
y = -30.5;  
text(x, y, 'Uncertainty reduction for future', 'FontSize', 13, 'HorizontalAlignment', 'center');
x = 0;  
y = -40.72;  
text(x, y, 'freezing day frequency under SSP245 (%)', 'FontSize', 13, 'HorizontalAlignment', 'center');
newFontSize = 13;
